...
namespace Mango.Analysis
{
    ...
    public static class ContenderAnalyzer
    {
        public static void AnalyzeAndReport(string directoryPath, string outputMode = "both")
        {
            ...
            {
                OutputToScreen(FileMetricsSummarys);
            }

            if (outputMode == "file" || outputMode == "both")
            {
                string rtfFilePath = Path.Combine(directoryPath, "CombinedAnalysisReport.rtf");
        ...
        {
            var metricsSummary = new Dictionary<string, (int PassCount, int TotalCount, List<string> Passers, List<string> Failers)>();
            ...
            foreach (var line in fileContent)
            {
                ...
                else if (line.StartsWith("- "))
                {
                    ...
                    bool passed = line.Contains("PASS");
        ...
        private static void OutputToScreen(List<FileMetricsSummary> results)
        {
            foreach (var result in results)
            {
                ...
                {
                    string passRate = $"{metric.Value.PassCount}/{metric.Value.TotalCount} ({(double)metric.Value.PassCount / metric.Value.TotalCount:P1})";
        ...
        private static void OutputToRTF(List<FileMetricsSummary> results, string rtfFilePath)
        {
            ...
            foreach (var result in results)
            {
                ...
                foreach (var metric in result.MetricsSummary)
                {
                    ...
                    foreach (var passer in metric.Value.Passers.Take(5))
                    {
                        rtfBuilder.AppendLine($@"    {passer}\par");
                    }

                    rtfBuilder.AppendLine($@"  Top Failers:\par");
                    foreach (var failer in metric.Value.Failers.Take(5))
                    {
                        rtfBuilder.AppendLine($@"    {failer}\par");
                    }
                }
            }
    ...
    public class FileMetricsSummary
    {
        ...
        public Dictionary<string, (int PassCount, int TotalCount, List<string> Passers, List<string> Failers)> MetricsSummary { get; set; }
    }
}
