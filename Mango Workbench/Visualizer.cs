...
public static class Visualizer
{
    ...
    public static List<string> Format(byte[] input, byte[] transformed, string mode = "BITS", int rows = 10,
        int columns = 80, int offset = 0, string format = "HEX")
    {
        var rowsList = new List<string>();

        var bytesPerColumn = mode.Equals("BITS", StringComparison.OrdinalIgnoreCase) ? 1 : columns;
        var totalLength = Math.Min(input.Length, transformed.Length);
        var start = Math.Min(offset, totalLength);

        for (var row = 0; row < rows; row++)
        {
            var position = start + row * columns;
            ...
            for (var col = 0; col < columns; col++)
            {
                ...
                var transformedByte = transformed[index];

                if (mode.Equals("BITS", StringComparison.OrdinalIgnoreCase))
                {
                    var inputBits = Convert.ToString(inputByte, 2).PadLeft(8, '0');
                    var transformedBits = Convert.ToString(transformedByte, 2).PadLeft(8, '0');
                ...
                {
                    var transformedHex = transformedByte.ToString("X2");
...
#if false
        public static string Format(byte[] input, byte[] transformed, string mode = "BITS", int rows = 10, int columns =
 80, int offset = 0, string format = "HEX")
        {
            var builder = new StringBuilder();
