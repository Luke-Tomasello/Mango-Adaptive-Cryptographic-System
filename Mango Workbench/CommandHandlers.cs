...
namespace Mango.Workbench
{
    public partial class Handlers
    {
        ...
        public static (string, ConsoleColor) ProfileTransformsWorker(ExecutionEnvironment parentEnv, string[] args)
        {
            ...
            string jsonPath = "TransformProfileResults.json";
            Dictionary<string, double> resultCache = File.Exists(jsonPath)
            ...
            double performanceScore = ProfileTransformsCore(parentEnv, rounds);
            ...
            Console.WriteLine("\n=== Performance Results ===");
            foreach (var kv in resultCache.OrderBy(kv => kv.Value))
            {
                Console.WriteLine($"{kv.Key}: {kv.Value:F4} ms");
            }
        ...
        public static double ProfileTransformsCore(ExecutionEnvironment parentEnv, int rounds)
        {
            ...
            foreach (var kvp in transformRegistry)
            {
                ...
                byte inverseId = (byte)transform.InverseId;
        ...
        public static (string, ConsoleColor) BenchmarkTransforms(ExecutionEnvironment localEnv, string[] args)
        {
            ...
            {
                return ("Run benchmark transforms canceled by user.", ConsoleColor.Red);
            }
        ...
        public static (string, ConsoleColor) QueryHandler(ExecutionEnvironment localEnv, string[] args)
        {
            ...
            string key = args[0].ToLower(); // Normalize key to lowercase
            ...
            try
            {
                switch (key)
                {
                    case "inputtype":
                        ...
                        int chunkCount = localEnv.Globals.Input.Length / 4096;
                        ...
                        {
                            byte[] chunk = localEnv.Globals.Input.Skip(i * 4096).Take(4096).ToArray();
                            string first16Bytes = string.Join(" ", chunk.Take(16).Select(b => b.ToString("X2")));
        ...
        public static (string, ConsoleColor) GetSettingsHandler(string[] args)
        {
            ...
            if (compoundProperty != null)
            {
                ...
                var relatedValues = compoundAttribute.RelatedProperties
                    .Select(propName =>
                    {
                ...
                string derivedValue = relatedValues.Any() ? string.Join(", ", relatedValues) : "<unset>";
            ...
            if (property.PropertyType == typeof(OperationModes))
            {
                value = $"{value}";
            }
        ...
        public static (string, ConsoleColor) SetSettingsHandler(ExecutionEnvironment localEnv, string[] args)
        {
            if (args.Length < 2)
            {
                return ("<Yellow>Usage: set <key> <value></Yellow>", Console.ForegroundColor);
            }

            string key = args[0];
            string value = string.Join(" ", args.Skip(1)); // Combine all args after the key into a single string
            ...
            try
            {
                ...
                if (compoundProperty != null)
                {
                    ...
                    int requiredArguments = compoundAttribute.RelatedProperties.Length;

                    // 🚨 **If SCR is selected, enforce its constraints**
                    bool isSCR = values.Length > 0 && values[0].Equals("SCR", StringComparison.OrdinalIgnoreCase);
                    ...
                    for (int i = values.Length; i < requiredArguments; i++)
                    {
                        if (properties.TryGetValue(compoundAttribute.RelatedProperties[i], out var extraProperty))
                        {
                            object defaultValue = extraProperty.PropertyType.IsValueType
        ...
        public static (string, ConsoleColor) ListSettingsHandler(ExecutionEnvironment localEnv, string[] args)
        {
            ...
            if (!properties.Any())
            {
                return ("No accessible settings found.", ConsoleColor.Yellow);
            }
            ...
            foreach (var property in properties)
            {
                ...
                bool isPartOfCompound = properties.Any(p =>
            ...
            List<byte> userSequence, string[] args)
        {
            ...
            if (!results.Any())
            {
                return ("No valid sequences found.", ConsoleColor.Yellow);
            }
            ...
            byte[] inputCopy = localEnv.Globals.Input.ToArray();
            var inputRows = Visualizer.Format(inputCopy, inputCopy, mode: mode, rows: rows, columns: columns,
                offset: offset, format: format);

            foreach (var (row, rowIndex) in inputRows.Select((value, i) => (value, i)))
            {
                string label = rowIndex == 0 ? Field("Input", 20) : new string(' ', 20);
            ...
            foreach (var (encrypted, index) in results.Select((value, i) => (value, i)))
            {
                ...
                byte[] previousCopy = previousEncrypted.ToArray();
                var transformRows = Visualizer.Format(previousCopy, encrypted, mode: mode, rows: rows, columns: columns,
                    offset: offset, format: format);

                foreach (var (row, rowIndex) in transformRows.Select((value, i) => (value, i)))
                {
                    string label = rowIndex == 0 ? transformName : new string(' ', 20);
                    ColorConsole.WriteLine($"{label}{row}");
                }
        ...
        public static (string, ConsoleColor) RunComparativeAnalysis(ExecutionEnvironment localEnv, SequenceHelper.ParsedSequence parsedSequence)
        {
            ...
            try
            {
                ...
                byte[] MangoEncrypted = localEnv.Crypto.Encrypt(sequence.ToArray(), localEnv.Globals.Input);
                stopwatch.Stop();
                TimeSpan MangoTime = stopwatch.Elapsed;
                ...
                byte[] AESPayload = AesEncrypt(localEnv.Globals.Input, password: GlobalsInstance.Password,
                    out int saltLength,
                    out int paddingLength);
                stopwatch.Stop();
                TimeSpan AESTime = stopwatch.Elapsed;
                ...
                var analysisResults = localEnv.CryptoAnalysis.RunCryptAnalysis(
                    encryptedData: MangoPayload,
                    avalanchePayload: MangoAvalanchePayload,
                    keyDependencyPayload: MangoKeyDependencyPayload,
                    inputData: localEnv.Globals.Input);

                double aggregateScore = localEnv.CryptoAnalysis.CalculateAggregateScore(analysisResults, localEnv.Globals.UseMetricScoring, logFilePath: null);
                ...
                List<string> mangoHeader = GenerateHeader(
                    localEnv,
                    formattedSequence: formattedSequence,
                    analysisResults: analysisResults,
                    isReversible: true,
                    name: "Mango'",
                    options: UtilityHelpers.HeaderOptions.AllAnalysis | UtilityHelpers.HeaderOptions.Mode | UtilityHelpers.HeaderOptions.InputType |
                             UtilityHelpers.HeaderOptions.MetricScoring | UtilityHelpers.HeaderOptions.PassCount
                );
                ...
                analysisResults = localEnv.CryptoAnalysis.RunCryptAnalysis(
                    encryptedData: AESPayload,
                    avalanchePayload: AESAvalanchePayload,
                    keyDependencyPayload: AESKeyDependencyPayload,
                    inputData: localEnv.Globals.Input);

                aggregateScore = localEnv.CryptoAnalysis.CalculateAggregateScore(analysisResults, localEnv.Globals.UseMetricScoring, logFilePath: null);
                passCount = analysisResults.Count(result => result.Passed);
                totalMetrics = analysisResults.Count;

                // Display cryptanalysis report: Dummy sequence for AES (it doesn't execute our sequences)
                List<string> aesHeader = GenerateHeader(
                    localEnv,
                ...
                List<string> aesAnalysis = localEnv.CryptoAnalysis.CryptAnalysisReport(localEnv.Crypto, analysisResults);
                List<string> aesTiming = new List<string> { $"AES Encryption took: {AESTime.TotalMilliseconds} ms" };
        ...
        private static (string, ConsoleColor) RunAutoWeightTuner(ExecutionEnvironment localEnv, string babyMungeFile)
        {
            ...
            List<string> header = GenerateHeader(localEnv, title: "Auto Weight Tuner", name: null,
            ...
            var topTransforms = ExtractTopTransforms(babyMungeFile, maxSequences: 10);
            ...
            double bestScore = RunBabyMunge(localEnv, topTransforms, currentWeights, ref bestSequenceSoFar);
            ...
            foreach (var weightConfig in GenerateWeightVariations(FailedWeightConfigs, currentWeights))
            {
                ...
                string weightSignature = string.Join("|",
                ...
                double newScore = RunBabyMunge(localEnv, topTransforms, weightConfig, ref newBestSequence);
                var newMetrics = EvaluateMetrics(localEnv, topTransforms, weightConfig, ref newBestSequence);

                bool isOverallBetter = (newScore > bestScore);
                bool isStable = CheckMetricStability(localEnv, bestMetrics, newMetrics);
            ...
            foreach (var kvp in bestWeights)
            {
                ColorConsole.WriteLine($"- **{kvp.Key}** → `{kvp.Value:F4}`");
            }
            ...
            HashSet<string> FailedWeightConfigs, Dictionary<string, double> currentWeights)
        {
            ...
            const double minWeight = 0.05; // 🔹 Prevents metrics from going to zero

            List<Dictionary<string, double>> variations = new();

            foreach (var metric in currentWeights.Keys)
            {
                for (int i = 1; i <= maxVariationsPerMetric; i++)
                {
                    double adjustment = stepSize * i;
            ...
            string targetMetric, double adjustment)
        {
            ...
            double newValue = newWeights[targetMetric] + adjustment;
            ...
            double total = newWeights.Values.Sum();
            ...
            ref List<string> bestSequenceSoFar)
        {
            ...
            Dictionary<string, double> metricSums = localEnv.CryptoAnalysis.MetricsRegistry.Keys
            ...
            int sequenceCount = results.Count;
            if (sequenceCount == 0) return new Dictionary<string, double>(); // Prevent division by zero

            Dictionary<string, double> averagedMetrics = metricSums
        ...
        private static HashSet<string> ExtractTopTransforms(string babyMungeFile, int maxSequences = 10)
        {
            ...
            foreach (var line in File.ReadLines(babyMungeFile))
            {
                if (line.StartsWith("Sequence: "))
                {
                    ...
                    string sequencePart = line.Substring(9).Trim(); // Remove "Sequence: " prefix
            ...
            Dictionary<string, double> newMetrics)
        {
            ...
            {
                double oldValue = previousMetrics[metric];
                double newValue = newMetrics[metric];
                double threshold = GetMetricThreshold(localEnv, metric);
        ...
        {
            if (localEnv.CryptoAnalysis.MetricsRegistry.TryGetValue(metric, out var metricInfo))
            {
                return metricInfo.Baseline;
            }
            ...
            Dictionary<string, double> weightConfig, ref List<string> bestSequenceSoFar)
        {
            ...
            List<SequenceResult> results = MiniMunge(localEnv, topTransforms, ref bestSequenceSoFar);
            ...
            SequenceResult bestResult = results.OrderByDescending(r => r.AggregateScore).First();
            double newBestScore = bestResult.AggregateScore;

            // ✅ If this run found a better sequence, update the best one
            double currentBestScore = (bestSequenceSoFar.Count > 0) ? newBestScore : 0.0;
            ...
            ref List<string> bestSequenceSoFar)
        {
            ...
            List<byte> transformIds = transformList
                .Select(name =>
                {
            ...
            var sequences = GeneratePermutations(transformIds, length: 4).ToList();

            // ✅ If a best sequence exists, run it first
            if (bestSequenceSoFar != null && bestSequenceSoFar.Count > 0)
            {
                List<byte> bestTransformIds = bestSequenceSoFar
            ...
            byte[] inputData = GenerateTestInput(localEnv);
            ...
            foreach (var sequence in sequences)
            {
                ...
                System.Diagnostics.Debug.Assert(false, "Unimplemented: need to extract payload here.");
                ...
                List<string> sequenceStrings = seqHelper.GetNames(sequence.ToList());
                ...
                if (sequenceCount % 100 == 0 || pl.ShouldLog())
                {
                    pl.WriteLine($"🔄 Processed {sequenceCount} sequences...");
                }
            }
        ...
        public static List<CryptoAnalysis.AnalysisResult> EncryptAndAnalyze(ExecutionEnvironment localEnv, List<byte> sequence)
        {
            // Measure Mango Encryption Time
            byte[] MangoEncrypted = localEnv.Crypto.Encrypt(sequence.ToArray(), localEnv.Globals.Input);
            var MangoPayload = localEnv.Crypto.GetPayloadOnly(MangoEncrypted); // Extract payload for Mango encryption
        ...
        {
            Dictionary<string, double> currentWeights = GetCurrentWeights();
            Dictionary<string, double> newWeights = new(currentWeights);

            // ✅ Identify problem areas (over/under contribution)
            Dictionary<string, double> metricScores = EvaluateMetricImpact(transforms);
            double targetScore = 50.0; // Ideal balance point

            foreach (var (metric, score) in metricScores)
            {
                double deviation = Math.Abs(score - targetScore);
                if (deviation > 5.0) // ✅ Only adjust weights if deviation is significant
                {
                    double adjustment = deviation / 100.0; // Scale adjustment factor
        ...
        public static (string, ConsoleColor) MangoCipherHandler(ExecutionEnvironment localEnv, string[] args)
        {
            ...
            List<string> sequence = sequenceString.Split(" -> ").ToList();
        ...
        public static void CheckRecordFail(ExecutionEnvironment localEnv, List<CryptoAnalysis.AnalysisResult> analysisResults, List<byte> currentSequence, string failurekey)
        {
        ...
        public static (string, ConsoleColor) LogToScreenHandler(ExecutionEnvironment localEnv)
        {
            ...
            bool success = localEnv.CryptoAnalysis.LogToScreen(localEnv, localEnv.Globals.InputType, maxContenders: 5);
            ...
            if (!success)
            {
                return ("EvaluationSummary failed due to sanity check errors.", ConsoleColor.Red);
            }
        ...
        {
            string logFileName = GetContenderFilename(localEnv, 0);
            localEnv.CryptoAnalysis.LogToFile(localEnv, logFileName, maxContenders: 1000);
        ...
        {
            string filaname = "ContenderLog.txt";
            FileToSQL(filaname);
        ...
        public static (string, ConsoleColor) RunOptimizeGRHandler(ExecutionEnvironment localEnv, List<string> sequence, string[] args)
        {
            // 🔍 Step 1: Parse `-max N`
            int roundsMax = args
                ...
                .Select(arg =>
                {
                    if (int.TryParse(arg, out int val)) return val;
                    return -1;
                })
                .FirstOrDefault();
            ...
            if (roundsMax <= 0)
            {
                return ("❌ Missing or invalid parameter: -max <N> (e.g., -max 9)", ConsoleColor.Red);
            }
        ...
        public static (string, ConsoleColor) RunOptimizeGRCore(ExecutionEnvironment localEnv, SequenceHelper.ParsedSequence parsedSequence, int roundsMax)
        {
            SequenceHelper seqHelper = new(localEnv.Crypto);
            List<byte> sequence = seqHelper.GetIDs(parsedSequence);
            ...
            string formatted = seqHelper.FormattedSequence<string>(parsedSequence,
                SequenceFormat.ID | SequenceFormat.TRounds | SequenceFormat.RightSideAttributes, chunks: 2, indent: true);

            int roundsStart = 1;
            double bestScore = 0;
            int bestGR = roundsStart;
            string bestFormatted = formatted;
            List<CryptoAnalysis.AnalysisResult> bestMetrics = null;

            ColorConsole.WriteLine($"\n<white>🔧 Optimizing Global Rounds (Start: {roundsStart}, Max: {roundsMax})</white>\n");

            for (int rounds = roundsStart; rounds <= roundsMax; rounds++)
            {
                ...
                double score = localEnv.CryptoAnalysis.CalculateAggregateScore(results, localEnv.Globals.UseMetricScoring);
            ...
            if (bestMetrics != null)
            {
                var finalSeq = seqHelper.FormattedSequence<string>(parsedSequence,
                    SequenceFormat.ID | SequenceFormat.TRounds | SequenceFormat.InferGRounds | SequenceFormat.RightSideAttributes,
                    chunks: 2, indent: true);
        ...
        public static (string, ConsoleColor) RunSequence(ExecutionEnvironment localEnv, SequenceHelper.ParsedSequence parsedSequence)
        {
            SequenceHelper seqHelper = new(localEnv.Crypto);
            List<byte> sequence = seqHelper.GetIDs(parsedSequence);
            string formattedSequence = seqHelper.FormattedSequence<string>(parsedSequence,
            ...
            try
            {
                ...
                ColorConsole.WriteLine(
                    ...
                    Visualizer.Format(inputCopy, inputCopy, mode: mode, rows: rows, columns: columns,
                        format: format)[0]);
                ...
                byte[] inputForComparison = localEnv.Globals.Input.ToArray();
                ColorConsole.WriteLine(
                    Field("Encrypted Data", labelWidth) +
                    Visualizer.Format(inputForComparison, payload, mode: mode, rows: rows, columns: columns,
                        format: format)[0]);

                // Bit Comparison (ensure Format() doesn’t alter inputData)
                var bitComparisonRows = Visualizer.Format(inputForComparison, payload, mode: mode, rows: rows,
                ...
                byte[] inputForDecryptionComparison = localEnv.Globals.Input.ToArray();
                ColorConsole.WriteLine(
                    Field("Decrypted Data", labelWidth) +
                    Visualizer.Format(inputForDecryptionComparison, decrypted, mode: mode, rows: rows, columns: columns,
                        format: format)[0]);
                ...
                var analysisResults = localEnv.CryptoAnalysis.RunCryptAnalysis(
                    ...
                    avalanchePayload: MangoAvalanchePayload,
                    keyDependencyPayload: MangoKeyDependencyPayload,
                    inputData: localEnv.Globals.Input, logFilePath: null);
        ...
        {
            SequenceHelper seqHelper = new(localEnv.Crypto);
            List<byte> _sequence = seqHelper.GetIDs(parsedSequence);
            SequenceFormat format = SequenceFormat.ID | SequenceFormat.InferTRounds | SequenceFormat.InferGRounds |
            ...
            if (!permutations.Any())
            {
                return ("No permutations generated. Ensure your sequence is valid.", ConsoleColor.Red);
            }
            ...
            {
                Console.WriteLine(
                    $"Testing sequence: {new SequenceHelper(localEnv.Crypto).FormattedSequence(permutation, format, chunks: 2, indent: true)}");
            ...
            bool isReversible = true; // TestSequence() above already ensures reversibility
            ...
            List<string> commandHeader =
                GenerateHeader(localEnv, title: "Run Best Fit", name: null, options: UtilityHelpers.HeaderOptions.AllExecution);
            // ✅ Report all sections in **one call** (keeps them in the same output file)
            ReportHelper.Report(localEnv.Globals.ReportFormat,
                new List<string>[]
                {
        ...
        public class FileReadException : Exception
        {
            ...
            public FileReadException(string fileName, string message, Exception innerException = null)
                : base($"Error reading {fileName}: {message}", innerException)
            {
                FileName = fileName;
            }
        }
        public class NoSequencesFoundException : Exception
        {
            public string FileName { get; }

            public NoSequencesFoundException(string fileName)
                : base($"No sequences found in {fileName}")
            {
                FileName = fileName;
            }
        }
        ...
        public static Dictionary<string, List<(List<byte> Sequence, double AggregateScore, List<CryptoAnalysis.AnalysisResult> Metrics)>> CreateCandidateList(
    Dictionary<string, List<(List<byte> Sequence, double AggregateScore, List<CryptoAnalysis.AnalysisResult> Metrics)>> table)
        {
            Dictionary<string, List<(List<byte> Sequence, double AggregateScore, List<CryptoAnalysis.AnalysisResult> Metrics)>> refinedTable = new();

            foreach (var entry in table)
            {
                string dataType = entry.Key;
                List<(List<byte> Sequence, double AggregateScore, List<CryptoAnalysis.AnalysisResult> Metrics)> contenders = entry.Value;
                ...
                if (firstContender.Sequence == null)
                {
                    ...
                    refinedTable[dataType] = new List<(List<byte> Sequence, double AggregateScore, List<CryptoAnalysis.AnalysisResult> Metrics)>();
                    continue;
                }

                var metricNames = firstContender.Metrics.Select(m => m.Name).ToList(); // No hardcoded metric names

                List<(List<byte> Sequence, double AggregateScore, List<CryptoAnalysis.AnalysisResult> Metrics)> bestTransforms = new();
                ...
                foreach (string metric in metricNames)
                {
                    var bestTransform = contenders
                        ...
                        .Select(c => (Transform: c, MetricScore: c.Metrics.FirstOrDefault(m => m.Name == metric)?.Score ?? double.MinValue))
                        .OrderByDescending(x => x.MetricScore) // Sort by highest metric score
                        .FirstOrDefault();

                    if (bestTransform.Transform.Sequence != null)
                    {
                        bestTransforms.Add((bestTransform.Transform.Sequence, bestTransform.Transform.AggregateScore, bestTransform.Transform.Metrics));
