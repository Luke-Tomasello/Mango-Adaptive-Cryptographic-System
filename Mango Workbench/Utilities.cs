...
namespace Mango.Utilities
{
    ...
    {
        private static Dictionary<string, Dictionary<int, byte[]>> CutMatrixCache = new();
        private string key;
        ...
        private void Init(string file)
        {
            ...
            key = GenerateCutListKey(file);

            if (!CutMatrixCache.ContainsKey(key))
            {
                if (File.Exists(file))
                {
                    messages.Add($"⚠️ Cutlist key missing for {key}, generating from file...");
                ...
                {
                    messages.Add($"⚠️ No contender file found for {key}. Using fallback: ALL transforms remain valid (no cuts applied).");

                    CutMatrixCache[key] = new Dictionary<int, byte[]>();

                    foreach (var (id, info) in _cryptoLib.TransformRegistry)
                        CutMatrixCache[key][id] = new byte[4] { 1, 1, 1, 1 }; // everything is VALID (uncut)
                }
            }

            activeDataIndex = GetIndexFromFileName(file);
        }
        public bool IsCut(int transformId)
        {
            return CutMatrixCache.ContainsKey(key) &&
                   CutMatrixCache[key].ContainsKey(transformId) &&
                   CutMatrixCache[key][transformId][activeDataIndex] != 1;
        }
    }
    public partial class CutListHelper
    {
        ...
        public static void Compile(CryptoLib cryptoLib)
        {
            ...
            string searchPattern = "Contenders,-L?-P?-D?-MC-ST.txt";
            string directoryPath = _where;
        ...
        private static string GenerateCutListKey(string fileName)
        {
            ...
            string level = fileNameParts[1];        // L1, L2, etc..
            string passSount = fileNameParts[2];    // sequence must pass PassCount metrics to be considered
            string dataType = fileNameParts[3];     // Data type: Combined, Random, etc..
        ...
        static void ProcessContenderFile(string file)
        {
            try
            {
                messages.Add($"Processing: {file}");

                string key = GenerateCutListKey(file);
                ...
                if (!CutMatrixCache.ContainsKey(key))
                {
                    ...
                    foreach (var (id, info) in _cryptoLib.TransformRegistry)
                        CutMatrixCache[key][id] = new byte[4] { 0, 0, 0, 0 }; // default "cut"
                }
                ...
                if (lines.Count < 10)
                    messages.Add($"⚠️ Only {lines.Count} contender(s) found in {file}. Cut matrix will reflect limited data.");
                ...
                foreach (var id in _cryptoLib.TransformRegistry.Keys)
                {
                    ...
                    bool usedInTop10 = transformsInTopSequences.Contains(id);
                    CutMatrixCache[key][id][dataIndex] = usedInTop10 ? (byte)0x01 : (byte)0x00;
                }
        ...
        static void SanityCheck()
        {
            ...
            foreach (var filePath in contenderFiles)
            {
                string key = GenerateCutListKey(filePath);
                string dataType = GetDataTypeFromFileName(filePath);
                ...
                foreach (var transformId in transformsInTopSequences)
                {
                    ...
                    if (!CutMatrixCache[key].ContainsKey(transformId) || CutMatrixCache[key][transformId][dataIndex] != 0x01)
                        throw new Exception($"❌ Sanity Check Failed: Transform '{_cryptoLib.TransformRegistry[transformId].Name}' from {key} is missing or marked incorrectly.");
                }
            }

            messages.Add("✅ Sanity check completed. All top 10 transforms accounted for.");
        }
        static void ValidateAndFinalizeCutList()
        {
            foreach (var entry in CutMatrixCache)
            {
        ...
        {
            string outputPath = Path.Combine(AppContext.BaseDirectory, _dbname);
        ...
        {
            CutMatrixCache = new();
            string jsonPath = Path.Combine(AppContext.BaseDirectory, _dbname);
                ...
                try
                {
                    ...
                    if (parsed != null)
                    {
                        foreach (var entry in parsed)
                            CutMatrixCache[entry.Key] = entry.Value;
        ...
        {
            string searchPattern = "Contenders,-L?-P?-D?-M?-S?.txt";
            string directoryPath = _where;

            bool contenderFilesExist = Directory.Exists(directoryPath) &&
                                       Directory.GetFiles(directoryPath, searchPattern).Length > 0;

            string jsonPath = Path.Combine(AppContext.BaseDirectory, _dbname);
            bool cutlistJsonExists = File.Exists(jsonPath);
    ...
    public partial class CutListHelper
    {
        ...
        public static bool VerifyCutList(string contenderFileName, List<byte> transforms, out List<byte> diff_cutlist_vs_file, out List<byte> diff_cutlist_vs_table)
        {
            // Early exit if contender file doesn't exist (likely first-run scenario)
            if (!File.Exists(contenderFileName))
            {
                messages.Add($"⚠️ Skipping Cut List verification: {contenderFileName} not found (expected for new levels).");
            ...
            try
            {
                ...
                diff_cutlist_vs_file = sortedInput.Except(fileActual).Concat(fileActual.Except(sortedInput)).OrderBy(x => x).ToList();

                // Compare input vs table
                diff_cutlist_vs_table = sortedInput.Except(tableActual).Concat(tableActual.Except(sortedInput)).OrderBy(x => x).ToList();

                bool allMatch = !diff_cutlist_vs_file.Any() && !diff_cutlist_vs_table.Any();
        ...
        {
            string filePath = Path.Combine(_where, Path.GetFileName(contenderFileName));
            var transformsInTopSequences = new HashSet<int>();
        ...
        {
            string key = GenerateCutListKey(contenderFileName);
            string dataType = GetDataTypeFromFileName(contenderFileName);
            ...
            if (CutMatrixCache.ContainsKey(key))
            {
                foreach (var kvp in CutMatrixCache[key])
                {
    ...
    public class LocalEnvironment : IDisposable
    {
        ...
        public SequenceHelper.ParsedSequence ParsedSequence { get; }
            ...
            : this(localEnv) // ✅ Calls first overload to initialize base environment
        {
            ...
            {
                localEnv.Globals.UpdateSetting(key, value);
            }
        }
        ...
        public LocalEnvironment(ExecutionEnvironment localEnv, List<string> sequence)
        {
            ...
            int globalRounds = ParsedSequence.SequenceAttributes.TryGetValue("GR", out string grValue) && int.TryParse(grValue, out int parsedGR)
                ? parsedGR
            ...
            if (!success)
            {
                throw new InvalidOperationException($"Failed to set transform rounds: {errorMessage}");
            }
    ...
    public class StateManager
    {
        ...
        private static readonly List<(object caller, string fullCallerInfo, string stackTrace)> _transformPushTracking =
            new List<(object, string, string)>(); // ✅ Tracks who pushed, full caller info, and stack trace
        ...
        private static readonly object _stackLock = new object();       // 🔹 Shared lock for all stacks
        private readonly object _context = new object();                // 🔹 ensure push/pop must belong to the same instance
        ...
        private void _pushGlobal(string key, object newValue)
        {
            ...
            PropertyInfo? propInfo = typeof(GlobalsInstance).GetProperty(key, BindingFlags.Public | BindingFlags.Instance);
            if (propInfo == null)
            ...
            if (propInfo != null)
            {
                ...
                object convertedValue = sequenceHandler.ConvertValue(propInfo.PropertyType, newValue);
        ...
        private void _pushAllGlobals()
        {
            ...
            {
                object currentValue = propInfo.GetValue(_localEnv.Globals)!;
        ...
        private void _popAllGlobals()
        {
            ...
            for (int ix = 0; ix < _globalStateFrameSize; ix++)
            {
                var (key, previousValue) = _globalStateStack.Pop();
                _localEnv.Globals.UpdateSetting(key, previousValue);
            }
        ...
        private void _pushGlobalRounds(int new_value)
        {
            ...
            PropertyInfo? propInfo = typeof(CryptoLibOptions).GetProperty(GlobalRoundsSetting.Split('.')[1]);
            if (propInfo == null)
                throw new InvalidOperationException($"Property '{GlobalRoundsSetting}' not found.");

            int currentRounds = (int)propInfo.GetValue(_cryptoLib.Options)!;
        ...
        private void _popGlobalRounds()
        {
            ...
            int previousRounds = _globalRoundStack.Pop();
        ...
        public void PushAllTransformRounds()
        {
            lock (_stackLock)
            {
                var (caller, fullCallerInfo) = GetCallerInfo(_context, framesUp: 1);
                ...
                {
                    HandleError($"Stack imbalance detected BEFORE push. Expected a multiple of {_transformCount}, but found {_transformRoundStack.Count}.");
                }
        ...
        public void PopAllTransformRounds()
        {
            lock (_stackLock)
            {
                ...
                int available = _transformRoundStack.Count;
                int required = _transformCount;
                ...
                int index = _transformPushTracking.FindLastIndex(entry => entry.caller == caller);
                if (index != -1)
                {
        ...
        {
            int stackSize = _transformRoundStack.Count;
            if (stackSize < _transformCount)
            {
                HandleError($"Stack is unbalanced: Expected {_transformCount} elements, but only {stackSize} remain. This suggests a mismatch between push and pop operations.");
            }
    ...
    public class GlobalsInstance
    {
        ...
        [Handlers.GlobalSetting]
        public int MaxSequenceLen { get; set; } = 3;
        [Handlers.GlobalSetting]
        public InputType InputType { get; set; } = InputType.Random;
        [Handlers.GlobalSetting]
        public int PassCount { get; set; } = 0;
        [Handlers.GlobalSetting]
        public int DesiredContenders { get; set; } = 1000;
        [Handlers.GlobalSetting]
        public bool Quiet { get; set; } = true;
        [Handlers.GlobalSetting]
        public int FlushThreshold { get; set; } = 50000; // Number of items before flushing console output and registering contenders
        [Handlers.GlobalSetting]
        public bool SqlCompact { get; set; } = false;    // Compact true outputs SQL queries in CSV, otherwise, a line based format is used
        ...
        [Handlers.GlobalSetting]
        public OperationModes Mode { get; set; } = OperationModes.Cryptographic;

        #region Batch Mode Processing
        [Handlers.GlobalSetting(IsDebugOnly: false, IsInternal: true, IsNoSave: true)]       // need to be able to set this through the commandline, don't save
        ...
        [Handlers.GlobalSetting(IsDebugOnly: true)]
        ...
        public void Load()
        {
            ...
            {
                Save(); // Create default settings file if none exists
            }

            var json = File.ReadAllText(SettingsFile);
            var settings = JsonSerializer.Deserialize<Dictionary<string, JsonElement>>(json);

            HashSet<string> loadedSettings = new(StringComparer.OrdinalIgnoreCase); // ✅ Track all explicitly set keys (case-insensitive)
                         ...
                         .Where(p => p.IsDefined(typeof(Handlers.GlobalSettingAttribute))))
            {
                ...
                {
                    object value = property.GetValue(this); // Get the default value from Globals
        ...
        public void Save()
        {
            ...
            foreach (var property in properties)
            {
                ...
                if (attribute?.IsNoSave == true)
                {
                    continue; // 🔥 Skip saving `IsNoSave: true` settings
                }
        ...
        public void UpdateSetting(string key, object value)
        {
            ...
            if (_cryptoLib == null)
                throw new InvalidOperationException($"Cannot update setting '{key}' because CryptoLib instance is required but was null.");
            ...
            var sequenceHandler = new SequenceAttributesHandler(_localEnv);
            object convertedValue = sequenceHandler.ConvertValue(property.PropertyType, value);
        ...
        private void TriggerSpecialActions(string key, object value)
        {
            switch (key.ToLowerInvariant())
            {
                case "inputtype":
                    ...
                    switch (_localEnv.Globals.InputType)
                    {
                        ...
                        case InputType.Sequence:
                            ...
                            break;
                        default:
                            throw new InvalidOperationException($"Unknown InputType detected during adaptive rounds assignment: {_localEnv.Globals.InputType}");
                    }
                    break;
                ...
                case "rounds":
                    ...
                    {
                        if (_cryptoLib.Options == null)
                            throw new InvalidOperationException("CryptoLib.Options is null. Ensure it is properly initialized before setting Rounds.");
    ...
    public class ExecutionEnvironment
    {
        ...
        public ExecutionEnvironment(CryptoLibOptions options, bool allowSaving = false)
        {
            ...
            CryptoLibOptions clonedOptions = options.Dupe();
        ...
        : this(existingEnv.Crypto.Options, allowSaving: false)
        {
            ...
            foreach (var (key, value) in settings)
            {
                ...
                if (key == "InputType" && value.Length == 1)
                {
                    processedValue = Enum.GetNames(typeof(InputType))
                                     ...
                                     ?? throw new FormatException($"❌ ERROR: Unrecognized InputType '{value}' in settings file!");
                }
                else if (key == "Mode" && value.Length == 1)
                {
                    processedValue = Enum.GetNames(typeof(OperationModes))
                                         .FirstOrDefault(name => name.StartsWith(value, StringComparison.OrdinalIgnoreCase))
                                     ?? throw new FormatException($"❌ ERROR: Unrecognized Mode '{value}' in settings file!");
                }
                else if (key == "UseMetricScoring")
                {
                    if (value != "T" && value != "F")
                        throw new FormatException($"❌ ERROR: Invalid UseMetricScoring value in settings file! (Expected 'T' or 'F', got '{value}')");
                ...
                object convertedValue = sequenceHandler.ConvertValue(typeof(GlobalsInstance).GetProperty(key).PropertyType, processedValue);
                Globals.UpdateSetting(key, convertedValue);
            }
        }
        bool MemberExists(object obj, string memberName)
        {
            var type = obj.GetType();
            return type.GetField(memberName, BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic) != null
                   || type.GetProperty(memberName, BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic) != null;
        }
    ...
    public class SequenceAttributesHandler
    {
        ...
        public void ApplyAttributes(Dictionary<string, string> attributes)
        {
            foreach (var kvp in attributes)
            {
                ...
                PropertyInfo? globalSetting = typeof(GlobalsInstance).GetProperty(key, BindingFlags.Public | BindingFlags.Instance);
                if (globalSetting == null)
                    throw new ArgumentException($"Unknown sequence-wide property: {key}");

                // ✅ Convert the value properly before setting it
                object convertedValue = ConvertValue(globalSetting.PropertyType, value);
        ...
        public object ConvertValue(Type targetType, object value)
        {
            if (value == null)
            {
                if (targetType.IsValueType)
                {
                    ...
                    if (instance == null)
                        throw new InvalidOperationException($"❌ CRITICAL ERROR: Failed to create an instance of {targetType.Name}. This must be explicitly handled.");

                    // 🚀 If it's an enum, verify that the default value is valid
                    if (targetType.IsEnum && !Enum.IsDefined(targetType, instance))
                        throw new InvalidOperationException($"❌ CRITICAL ERROR: The default value '{instance}' is not a valid {targetType.Name} enum value.");
            ...
            if (targetType.IsEnum && value is string stringValue)
            {
                if (Enum.TryParse(targetType, stringValue, true, out object result))
                    return result;
                throw new ArgumentException($"❌ CRITICAL ERROR: Invalid enum value '{stringValue}' for type {targetType.Name}");
            }
        ...
        public object ConvertValue(Type targetType, string value)
        {
            ...
            {
                if (!Enum.TryParse(targetType, value, true, out object parsedEnum))
                    throw new ArgumentException($"Invalid value '{value}' for {targetType.Name}. Allowed values: {string.Join(", ", Enum.GetNames(targetType))}");
                return parsedEnum;
            }
    ...
    public static class TestInputGenerator
    {
        ...
        public static void InitializeInputData()
        {
            lock (_initLock)
            {
                ...
                string randoms_filename = "randoms.bin";
                string natural_filename = "Frankenstein.bin";
                string natural_source = "Frankenstein.txt";
                ...
                if (!File.Exists(natural_filename))
                {
                    if (!File.Exists(natural_source))
                        throw new FileNotFoundException($"❌ CRITICAL ERROR: {natural_source} not found. Cannot create {natural_filename}.");

                    string textContent = File.ReadAllText(natural_source);
        ...
        {
            if (!_isInitialized)
                throw new InvalidOperationException("❌ CRITICAL ERROR: Test input data has not been initialized. Call InitializeInputData() first.");

            byte[] sourceData = type switch
            {
        ...
        {
            return GenerateTestInput(4096, type: localEnv.Globals.InputType);
        }
        ...
        private static readonly Dictionary<OperationModes, Dictionary<string, double>> modeWeights = new()
    {
        ...
        public static void AdjustWeights(ExecutionEnvironment localEnv, OperationModes mode)
        {
            ...
            {
                if (combinedWeights.TryGetValue(key, out double weight))
                {
    ...
    public static class UtilityHelpers
    {
            ...
            List<(string name, int id, int tRounds)> parsedSequence) // 🚀 No GR needed
        {
            ...
            {
                return (false, "No valid transforms provided.");
            }
        ...
        {
            string failDBColor = localEnv.Globals.CreateMungeFailDB ? "Red" : "Green"; // Red if MungeFailDB is enabled
            ...
            return new List<string>
            {
                ...
                $"<Green>Database: {UtilityHelpers.GetFailDBFilename(localEnv, "MungeFailDB,")}</Green>",         // Name of the MungeFailDB file
        ...
        public static List<string> GetMungeTitle(string headerTitle, string name = null)
        {
            ...
            string borderLine = new string('=', titleLength + 10); // Matches title length with padding
        ...
        public static List<string> GetMungeTail(List<string> headerLines)
        {
            ...
            int maxLength = headerLines.Max(line => ColorConsole.RemoveColorTags(line).Length);
            string borderLine = new string('=', maxLength); // Matches longest title length
        ...
        public static string GetContenderFilename(ExecutionEnvironment localEnv, int sequenceLength, string extension = "txt")
        {
        ...
        private static string GenerateFilename(ExecutionEnvironment localEnv,
           ...
           string extension = "")
        {
            int passCount = localEnv.Globals.PassCount;
            char dataType = localEnv.Globals.InputType.ToString()[0]; // ✅ 1st Char of InputType
            char mode = localEnv.Globals.Mode.ToString()[0];         // ✅ 1st Char of Mode
            char useMetricScoring = localEnv.Globals.UseMetricScoring ? 'T' : 'F';

            // ✅ Keep the prefix (e.g., "Contenders," or "MungeFailDB,")
            string lengthSegment = sequenceLength.HasValue ? $"-L{sequenceLength}" : "";

            string basename = $"{prefix}{lengthSegment}-P{passCount}-D{dataType}-M{mode}-S{useMetricScoring}";
        ...
        public static double GetAggregateScore(string filename, int contender)
        {
            ...
            try
            {
                ...
                string pattern = $@"Contender #{contender}\s+Sequence:\s*(.*?)\s+Aggregate Score:\s*(\d+(\.\d+)?)";
                Match match = Regex.Match(fileContent, pattern, RegexOptions.Singleline); // Singleline is important!
                ...
                if (match.Success)
                {
                    ...
                    {
                        Console.WriteLine($"Error: Could not parse Aggregate Score for Contender #{contender} in {filename}.");
        ...
        public static string GetSequence(string filename, int contender)
        {
            ...
            try
            {
                ...
                string pattern = $@"Contender #{contender}\s+Sequence:\s*(.*?)\s+Aggregate Score:";
        ...
        public static Dictionary<string, string> GetEnvironmentSettings(string filename)
        {
            ...
            if (settings.Count != 5)
                throw new InvalidOperationException("Missing required settings in filename. Expected 5 settings (MaxSequenceLen, PassCount, InputType, Mode, UseMetricScoring).");
        ...
        public static Dictionary<string, string> GenerateEnvironmentSettings(string[] args)
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                else if (arg.Contains("="))
                {
                    ...
                    if (parts.Length == 2)
                    {
                        string key = parts[0].TrimStart('-'); //Remove leading -
            ...
            string[] requiredKeys = new string[] { "MaxSequenceLen", "PassCount", "InputType", "Mode", "UseMetricScoring" };
            foreach (string key in requiredKeys)
            {
                if (!settings.ContainsKey(key))
                    throw new InvalidOperationException($"Missing required setting: {key}.  Expected in the format -{key}=value.");
            }
        ...
        public static Dictionary<string, string> GetEnvironmentSettings(ExecutionEnvironment exeEnv)
        {
            ...
            if (settings.Count != 5)
                throw new InvalidOperationException("Missing required settings in ExecutionEnvironment. Expected 5 settings (MaxSequenceLen, PassCount, InputType, Mode, UseMetricScoring).");

            return settings;
        }

        /// <summary>
        /// Attempts to extract the original date (YYMM_DDD) from the filename.
        /// </summary>
        /// <param name="filename">The filename to parse.</param>
        /// <returns>The extracted date string or null if not found.</returns>
        private static string? ExtractDateFromFilename(string filename)
        {
            var match = Regex.Match(filename, @"\b(\d{4}_\d{3})\b"); // Matches YYMM_DDD pattern
        ...
        {
            string defaultPattern = "Contenders,-L4-P6-D?-MC-ST.txt";
            string resolvedPattern = defaultPattern;
        ...
        public static List<byte[]> GetTopContendersAsIDs(ExecutionEnvironment env, string pattern, int contenders, int transforms)
        {
            ...
            if (files.Length == 0)
            {
                throw new FileNotFoundException("No contender files found. You must munge first.");
            }

            List<byte[]> topContenders = new List<byte[]>();
            SequenceHelper seqHelper = new SequenceHelper(env.Crypto); // ✅ Helper to convert sequences to IDs
                ...
                try
                {
                    using (StreamReader reader = new StreamReader(file))
                    {
                        ...
                        while ((line = reader.ReadLine()) != null)
                        {
                            ...
                            {
                                string sequence = line.Substring(9).Trim();
                                ...
                                {
                                    throw new InvalidOperationException($"Sequence in file {file} has only {sequenceList.Count} transforms, but {transforms} were requested.");
                                }
                                ...
                                byte[] sequenceIds = seqHelper.GetIDs(sequenceList).ToArray();
            ...
            if (topContenders.Count < contenders)
            {
                throw new Exception($"Only {topContenders.Count} contender sequences found, but {contenders} were requested.");
            }
        ...
        private static string[] GetMungeFiles(string[] args, string defaultPattern)
        {
            ...
            string[] allFiles = Directory.GetFiles(Directory.GetCurrentDirectory(), "Contenders,-L?-P?-D?-M?-S?.txt");
            ...
            var validParams = new Dictionary<string, string>
    {
        ...
        { "S", "T" }        // Default Metric Scoring = True
            ...
            bool hasArgs = args.Any(arg => arg.StartsWith("-"));
            ...
            foreach (var arg in args)
            {
                if (arg.StartsWith("-"))
                {
                    string key = arg.Substring(1, 1).ToUpper(); // Extract single-character key
                    string value = arg.Substring(2).ToUpper(); // Extract value
            ...
            string regexPattern = $@"^Contenders,-L{validParams["L"]}-P{validParams["P"]}-D{validParams["D"]}-M{validParams["M"]}-S{validParams["S"]}\.txt$";
        ...
        public static bool VerifyMungeFile(string[] files, out string message, params string[] requiredFlags)
        {
            ...
            {
                string seedValue = ExtractFlagValue(files[0], flag);
                if (seedValue == null)
                {
            ...
            foreach (string file in files)
            {
                foreach (string flag in requiredFlags)
                {
                    string detectedValue = ExtractFlagValue(file, flag);
                    if (!string.Equals(detectedValue, seedValues[flag], StringComparison.OrdinalIgnoreCase))
                    {
                        message = $"❌ Inconsistent `{flag}` values detected! Expected: {seedValues[flag]} but found: {detectedValue} in file: {file}";
                        return false;
                    }
                }
            }
        ...
        private static string ExtractFlagValue(string filename, string flag)
        {
            ...
            int index = filename.IndexOf(flag, StringComparison.OrdinalIgnoreCase);
            ...
            int end = start;
            while (end < filename.Length && char.IsLetterOrDigit(filename[end]))
            {
                end++;
            }
        ...
        public static class MungeStatePersistence
        {
            ...
            public static MungeState RestoreMungeState(string saveFileName)
            {
                try
                {
                    if (!File.Exists(saveFileName))
                    {
                        return null; // Return null if there's no saved state
                    }
            ...
            public class MungeState
            {
                ...
                public byte[] Transforms { get; set; }
                public byte[] Sequence { get; set; }
            }
        }


        public static (byte[] MangoAvalanchePayload, byte[]? AESAvalanchePayload, byte[] MangoKeyDependencyPayload, byte[]? AESKeyDependencyPayload)
                ...
                bool processAES = false)
        {
            ...
            if (processAES)
            {
                AESAvalanchePayload = AesEncrypt(ModifiedInput, password: password, out int saltLength, out int paddingLength);
            ...
            if (processAES)
            {
                AESKeyDependencyPayload = AesEncrypt(localEnv.Globals.Input, password: ModifiedPassword, out int saltLength, out int paddingLength);
        ...
        public static byte[] ModifyInput(byte[] reverseSequence, byte[] input)
        {
            ...
            long hashValue = BinaryPrimitives.ReadInt64LittleEndian(reverseHash); // Convert first 8 bytes to a long

            int totalBits = input.Length * 8; // Total number of bits in the input
            int bitToFlip = (int)(Math.Abs(hashValue) % totalBits); // Map hash to a valid bit index

            // Create a copy of the input and flip the calculated bit
            byte[] modifiedInput = (byte[])input.Clone();
        ...
        public static byte[] AesEncrypt(byte[] input, string password, out int saltLength, out int paddingLength)
        {
            ...
            using (var deriveBytes = new Rfc2898DeriveBytes(password, salt, 100000))
            {
                ...
                byte[] iv = deriveBytes.GetBytes(16);  // AES block size (16 bytes)
                ...
                using (var aes = Aes.Create())
                {
                    ...
                    {
                        byte[] encryptedData = encryptor.TransformFinalBlock(input, 0, input.Length);

                        // Calculate padding length
                        paddingLength = encryptedData.Length - input.Length;

                        // Prepend the salt to the encrypted data for decryption later
        ...
        public static byte[] ExtractAESPayload(byte[] encryptedData, int saltLength, int paddingLength)
        {
            ...
            int coreLength = encryptedData.Length - saltLength - paddingLength;
            ...
            string additionalInfo = null) // 🆕 New additional info parameter
        {
            ...
            string headerTitle = string.IsNullOrWhiteSpace(name) ? $"{title}" : $"{name} {title}";
            // ✅ Remove redundant spaces to avoid empty headers while preserving spacing within words.
            string titleLine = $"===== {headerTitle} =====".Replace("  ", "");
            string separator = new string('=', titleLine.Length);

            // 🔹 Create a list to store formatted output
            List<string> output = new List<string> { $"<Yellow>{titleLine}</Yellow>" };
            ...
            if (options.HasFlag(HeaderOptions.MetricScoring))
                output.Add($"<Gray>Metric Scoring:</Gray> <Green>{(localEnv.Globals.UseMetricScoring ? "Enabled" : "Disabled")}</Green>");
            ...
            {
                double aggregateScore = localEnv.CryptoAnalysis.CalculateAggregateScore(analysisResults, localEnv.Globals.UseMetricScoring);
                int passCount = analysisResults.Count(result => result.Passed);
                int totalMetrics = analysisResults.Count;
                ...
                if (options.HasFlag(HeaderOptions.Reversibility))
                    output.Add($"<Gray>Reversibility:</Gray> <{color}>[{(isReversible == true ? "PASS" : "FAIL")}]</{color}>");

                if (options.HasFlag(HeaderOptions.PassCount))
                    output.Add($"<Gray>Pass Count:</Gray> <Green>{passCount} / {totalMetrics}</Green>"); // Pass count is always last
            }
        ...
        public static void FileToSQL(string logFilePath)
        {
            ...
            using (var contenderInsert = new SQLiteCommand(
                "INSERT INTO Contenders (Sequence, AggregateScore, PassCount) VALUES (@Sequence, @AggregateScore, @PassCount);", connection))
            using (var metricInsert = new SQLiteCommand(
                "INSERT INTO Metrics (ContenderId, Name, Passed, Value, Threshold, Notes) VALUES (@ContenderId, @Name, @Passed, @Value, @Threshold, @Notes);", connection))
            {
    ...
    CREATE TABLE IF NOT EXISTS Metrics (
        ...
        private static void InsertContenderAndMetrics(Contender contender, SQLiteCommand contenderInsert, SQLiteCommand metricInsert)
        {
            // Insert Contender
            ...
            long contenderId = contenderInsert.Connection.LastInsertRowId;
        ...
        // Enhanced Command Matching Helper
        public static string FindClosestCommand(string input, IEnumerable<string> validCommands, int maxDistance)
        {
            var exactOrStartsWith = validCommands
                .Where(cmd => cmd.StartsWith(input, StringComparison.OrdinalIgnoreCase))
                .OrderBy(cmd => cmd.Length) // Shortest valid match
                .FirstOrDefault();

            if (!string.IsNullOrEmpty(exactOrStartsWith)) return exactOrStartsWith;

            return validCommands
                .Select(cmd => new { Command = cmd, Distance = MultiWordDistance(input, cmd) })
        ...
        public static int MultiWordDistance(string input, string command)
        {
            ...
            int totalDistance = 0;
            int maxLength = Math.Max(inputTokens.Length, commandTokens.Length);

            for (int i = 0; i < maxLength; i++)
            {
                string inputToken = i < inputTokens.Length ? inputTokens[i] : "";
                string commandToken = i < commandTokens.Length ? commandTokens[i] : "";
        ...
        public static void LogIfEnabled(ExecutionEnvironment localEnv, DebugFlags flag, string message, int sequenceLength = 0)
        {
            if ((DebugLevel & flag) != 0)
            {
                ...
                {
                    string filename = GetContenderFilename(localEnv, sequenceLength, "log");
                    string cleanMessage = ColorConsole.RemoveColorTags(message);
        ...
        public static byte[] GenerateReverseSequence(CryptoLib cryptoLib, byte[] forwardSequence)
        {
            ...
            byte[] reverseSequence = forwardSequence
        ...
        public static IEnumerable<byte[]> GeneratePermutations(List<byte> transformIds, int length)
        {
            IEnumerable<byte[]> Generate(byte[] sequence)
            {
                ...
                foreach (var transformId in transformIds)
                {
                    ...
                    {
                        yield return result; // ✅ Lazily yields next permutation
                    }
                }
            }
        ...
        private static IEnumerable<byte[]> ResumeSafePermutationGenerator(
            IEnumerable<byte[]> permutations,
            byte[] resumeSequence)
        {
            bool found = resumeSequence == null || resumeSequence.Length == 0;

            foreach (var seq in permutations)
            {
                if (!found)
                {
                    if (seq.SequenceEqual(resumeSequence))
                    {
                        found = true; // ✅ Found saved point
                    }
                    else
                    {
                        continue; // ⏩ Keep fast-forwarding
                    }
                }

                yield return seq; // Normal flow resumes here
            }
        }
        ...
        {
            public static IEnumerable<byte[]> GenerateMetaSequenceTransformPairs(List<byte[]> metaSequences, List<byte> transforms, int transformsToAdd)
            {
                foreach (byte[] metaSequence in metaSequences)
                {
                    foreach (List<byte> transformCombination in GenerateCombinations(transforms, transformsToAdd))
                    {
            ...
            private static IEnumerable<List<byte>> GenerateCombinations(List<byte> transforms, int length)
            {
                ...
                foreach (byte transform in transforms)
                {
                    foreach (List<byte> combination in GenerateCombinations(transforms, length - 1))
                    {
            ...
            {
                int combinations = (int)Math.Pow(transforms.Count, transformsToAdd);
        ...
        public static IEnumerable<T[]> GenerateLimitedRepetitionSequences<T>(IEnumerable<T> items, int length, int repetitions = 1)
        {
            ...
            return Permute(new List<T>(), length, itemCounts);

            IEnumerable<T[]> Permute<T>(List<T> current, int remaining, Dictionary<T, int> itemCounts)
            {
                ...
                foreach (var kvp in itemCounts.Where(kvp => kvp.Value > 0))
                {
                    ...
                    foreach (var perm in Permute(current, remaining - 1, itemCounts)) // Decrement remaining
                    {
                        yield return perm;
                    }
        ...
        public static class PermutationCounter
        {
            public static long CountLimitedRepetitionSequences<T>(IEnumerable<T> items, int length, int repetitions = 1)
            {
                ...
                int numDistinctItems = distinctItems.Count;
                ...
                if (length > numDistinctItems * repetitions)
                {
                    return 0;
                }
        ...
        public static IEnumerable<SequenceRoundConfig> GenerateSequencesAndRoundConfigs(List<byte> userSequence, int sequenceLength, int repetitions)
        {
        ...
        {
            for (int i = 0; i < Math.Pow(MaxRounds, sequenceLength); i++) // Uses MaxRounds
            {
        ...
        public static string ReadConsoleBlock(int timeoutMilliseconds = 150, bool trimBlankLines = true)
        {
            ...
            DateTime lastLineTime = DateTime.UtcNow;
        ...
        {
            byte upper = (byte)char.ToUpper((char)@byte);
        ...
        public static bool AreEqualWithF10Formatting(double a, double b)
        {
            ...
            string formattedA = a.ToString("F10");
            string formattedB = b.ToString("F10");
        ...
        public static void BenchmarkAllTransforms(ExecutionEnvironment localEnv)
        {
            ...
            using (var localStatEnvironment = new LocalEnvironment(localEnv))
            {
                ...
                {
                    int transformId = kvp.Key;
                    var transform = kvp.Value;

                    byte[] singleTransform = new byte[] { (byte)transformId };
                    byte[] reverseTransform = new byte[] { (byte)transform.InverseId };

                    // Encrypt timing
                    var sw = Stopwatch.StartNew();
                    byte[] encrypted = localEnv.Crypto.Encrypt(singleTransform, sampleInput);
                    sw.Stop();
                    double encryptTime = sw.Elapsed.TotalMilliseconds / localEnv.Globals.Rounds;

                    // Decrypt timing
                    sw.Restart();
                    byte[] decrypted = localEnv.Crypto.Decrypt(reverseTransform, encrypted);
                    sw.Stop();
                    double decryptTime = sw.Elapsed.TotalMilliseconds / localEnv.Globals.Rounds;

                    double timePerOp = (encryptTime + decryptTime) / 2;
                    totalTime += timePerOp;
            ...
            string totalSummary = $"Total Benchmark Time Across All Transforms: {totalTime:F4} ms";
            results.Add("");
            ...
            // Write to TXT
            string txtPath = "TransformBenchmarkResults.txt";
            try
            {
            ...
            // Write to JSON
            string jsonPath = "TransformBenchmarkResults.json";
            try
            {
        ...
        public static void EstablishCurrentBenchmarkTime(ExecutionEnvironment localEnv)
        {
            ...
            using (var localStatEnvironment = new LocalEnvironment(localEnv))
            {
                ...
                byte[] encrypted = localEnv.Crypto.Encrypt(singleTransform, sampleInput);
                sw.Stop();
                double encryptTime = sw.Elapsed.TotalMilliseconds / localEnv.Globals.Rounds; // Divide by rounds

                // Decrypt timing
                sw.Restart();
                byte[] decrypted = localEnv.Crypto.Decrypt(reverseTransform, encrypted);
                sw.Stop();
                double decryptTime = sw.Elapsed.TotalMilliseconds / localEnv.Globals.Rounds; // Divide by rounds
