...
public static class QueryLibrary
{
    public static List<Contender> GetAllContenders()
    {
        ...
        while (reader.Read())
        {
            ...
            // ✅ Only add metric if a MetricName exists
            if (!reader.IsDBNull(reader.GetOrdinal("MetricName")))
            {
                ...
                contenders[sequence].Metrics.Add(metric);
            }
        }
...
public static class MangoSQLConsole
{
    ...
    public static (string, ConsoleColor) ListContenders(string target, string orderBy, int limit)
    {
        ...
        foreach (var contender in contenders)
            result.AppendLine(
                $"Sequence: {contender.Sequence}, Score: {contender.AggregateScore:F4}, Pass Count: {contender.PassCount}");
    ...
    {
        if (args.Contains("/h")) return Help("GOOD");
        ...
        var type = tokens[1]; // Transform or Sequence
        var metric = tokens[3];
        var limit = args.Contains("LIMIT", StringComparison.OrdinalIgnoreCase) ? ExtractLimit(args) : 10;
        var minThreshold = args.Contains("MIN", StringComparison.OrdinalIgnoreCase)
            ? QueryLibrary.ExtractMinThreshold(args)
            : null;
        var isVerbose = args.Contains("VERBOSE", StringComparison.OrdinalIgnoreCase);
        var showFailedOnly = args.Contains("ShowFailedOnly", StringComparison.OrdinalIgnoreCase);
        var highlightCriticalMetrics = args.Contains("HighlightCriticalMetrics", StringComparison.OrdinalIgnoreCase);

        // Query the database
        var contenders = QueryLibrary.GetAllContenders();
        contenders = QueryLibrary.ApplyCondition(contenders, $"{metric} PASS");
        if (minThreshold.HasValue) contenders = QueryLibrary.ApplyThreshold(contenders, metric, minThreshold.Value);
        contenders = QueryLibrary.ApplyLimit(contenders, limit);
        ...
        foreach (var contender in contenders)
        {
            ...
            {
                result.AppendLine(
                    $"{rank}. {contender.Sequence}: Score: {contender.AggregateScore:F4}, Pass Count: {contender.PassCount}");
            }
    ...
    {
        if (args.Contains("/h")) return Help("TOUGH");
        ...
        foreach (var contender in contenders)
        {
            ...
            result.AppendLine(
                $"  <Green>Score:</Green> {contender.AggregateScore:F4}, <Green>Pass Count:</Green> {contender.PassCount}, <Green>Details:</Green>");
            foreach (var metricData in contender.Metrics)
            {
                if (showFailedOnly && metricData.Passed) continue;
    ...
    public static (string, ConsoleColor) FindContenders(string target, string condition, string orderBy, int limit)
    {
        ...
        while (contendersReader.Read())
        {
            ...
            using var metricsCommand =
                new SQLiteCommand(
                    "SELECT Name, Value, Threshold, Passed FROM Metrics WHERE ContenderId = @ContenderId;", connection);
    ...
    public static (string, ConsoleColor) ExecuteRawSql(string query, ExecutionEnvironment localEnv)
    {
        ...
        var result = new StringBuilder();
        var isVerbose = !localEnv.Globals.SqlCompact;
    ...
    public static (string, ConsoleColor) QueryMetaLanguageHandler(string input, ExecutionEnvironment localEnv)
    {
        ...
        return command switch
        {
            ...
            "FIND" => FindContenders(
                "contenders",
                ExtractCondition(input) ?? string.Empty,
                ExtractOrderBy(input) ?? string.Empty,
                ExtractLimit(input)
            ),
            "LIST" => ListMetrics(localEnv),
            "SHOW" => tokens.Length >= 4 && tokens[1].ToUpperInvariant() == "METRICS" &&
                      tokens[2].ToUpperInvariant() == "FOR"
                ? ShowMetricsForContender(tokens[3])
                : ("Usage: SHOW metrics FOR contender <Id>", ConsoleColor.Yellow),
            "COUNT" => CountContenders("contenders", ExtractCondition(input) ?? string.Empty),
    ...
    public static (string, ConsoleColor) Help(string? command = null)
    {
        if (string.IsNullOrEmpty(command))
        {
            ...
            foreach (var cmd in HelpText.Keys) allHelp.AppendLine($"- {cmd}");
        ...
        if (HelpText.TryGetValue(command, out var helpContent)) return (helpContent, ConsoleColor.Green);
    ...
    private static int ExtractLimit(string query)
    {
        ...
        return match.Success && int.TryParse(match.Groups[1].Value, out var limit) ? limit : int.MaxValue;
    }
}